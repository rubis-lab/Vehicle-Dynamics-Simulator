...
tControlCmd	CmdControlRef[] = {
    ...
    /*** kswe ***/
    , {HM_ATT_SCC_CMD,  GFCTRL_TYPE_NOT_AFFECTED, -1, NULL, 0.0, 0.0, NULL, 0.0, NULL, 0.0, NULL, 0.0, NULL, 0.0, NULL, 0.0},
    {HM_ATT_LKAS_CMD,  GFCTRL_TYPE_NOT_AFFECTED, -1, NULL, 0.0, 0.0, NULL, 0.0, NULL, 0.0, NULL, 0.0, NULL, 0.0, NULL, 0.0}
    /************/
};
...
HmReadPrefs(int index)
{
    ...
    /*** kswe ***/
    prm = GfParmGetStr(PrefHdle, sstring, HM_ATT_SCC, Yn[HCtx[idx]->ParamScc]);
    if (strcmp(prm, Yn[0]) == 0) {
	HCtx[idx]->ParamScc = 1;
    } else {
	HCtx[idx]->ParamScc = 0;
    }
    prm = GfParmGetStr(PrefHdle, sstring, HM_ATT_LKAS, Yn[HCtx[idx]->ParamLkas]);
    if (strcmp(prm, Yn[0]) == 0) {
	HCtx[idx]->ParamLkas = 1;
    } else {
	HCtx[idx]->ParamLkas = 0;
    }
   /*************/
