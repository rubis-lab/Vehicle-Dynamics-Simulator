...
static void common_drive(int index, tCarElt* car, tSituation *s)
{
	...
	((cmd[CMD_SCC].type == GFCTRL_TYPE_SKEYBOARD) && skeyInfo[cmd[CMD_SCC].val].edgeUp))
	{
		...
		torcs_output[CC_TRIGGER] = (float)HCtx[idx]->ParamScc;
		if(torcs_output[CC_TRIGGER] > 0.5)
		{
			torcs_output[TARGET_SPEED] = car->_speed_x*3.6;
			switch(HCtx[idx]->drivetrain)
			{
				case DRWD:
					torcs_output[TARGET_WHEEL_SPEED] = car->_wheelSpinVel(REAR_RGT);
					break;
				case DFWD:
					torcs_output[TARGET_WHEEL_SPEED] = car->_wheelSpinVel(FRNT_RGT);
					break;
				case D4WD:
					torcs_output[TARGET_WHEEL_SPEED] = (car->_wheelSpinVel(REAR_RGT)+car->_wheelSpinVel(FRNT_RGT))/2;
					break;
				default:
					break;			
			}
			...
			// for experiments
			torcs_output[TARGET_WHEEL_SPEED] = torcs_output[TARGET_WHEEL_SPEED]*80.0/torcs_output[TARGET_SPEED]; 
			torcs_output[TARGET_SPEED] = 80.0;
		}
	}
	...
	((cmd[CMD_LKAS].type == GFCTRL_TYPE_SKEYBOARD) && skeyInfo[cmd[CMD_LKAS].val].edgeUp))
	{
		...
		torcs_output[LKAS_TRIGGER] = (float)HCtx[idx]->ParamLkas;
	}
	...
	// LKAS
	if(user_input[STEER_MODE] < 0.5)	// calculate STEER_VALUE
	{
	...
	else
	{
		...
		NORM_PI_PI(angle);
//		angle -= SC*car->_trkPos.toMiddle/car->_trkPos.seg->width;	// Road Keeping Assist System
		if(car->_trkPos.toMiddle < 0.0)		// actual LKAS
		{
			angle -= SC*((car->_trkPos.toRight+car->_trkPos.toMiddle)/2.0)/(car->_trkPos.seg->width/2.0);	// right lane
			dist = (car->_trkPos.toRight+car->_trkPos.toMiddle)/2.0;
		}
		else
		{
			angle -= SC*((car->_trkPos.toMiddle-car->_trkPos.toLeft)/2.0)/(car->_trkPos.seg->width/2.0);	// left lane
			dist = (car->_trkPos.toMiddle-car->_trkPos.toLeft)/2.0;
		}
	}

	// Cruise Control
	float gr2[8] = {-2.0, 3.9, 2.9, 2.3, 1.87, 1.68, 1.54, 1.46};
	float pi2 = 2.0*3.1415;
	float rm = car->_enginerpmRedLine;
	switch(HCtx[idx]->drivetrain)
	{
		case DRWD:
			torcs_output[WHEEL_SPEED] = car->_wheelSpinVel(REAR_RGT);
			break;
		case DFWD:
			torcs_output[WHEEL_SPEED] = car->_wheelSpinVel(FRNT_RGT);
			break;
		case D4WD:
			torcs_output[WHEEL_SPEED] = (car->_wheelSpinVel(REAR_RGT)+car->_wheelSpinVel(FRNT_RGT))/2;
			break;
		default:
			break;			
	}
	float re = torcs_output[TARGET_WHEEL_SPEED]/pi2 * 6.0 * gr2[car->_gear] / rm;
	if(re > 1.0)
		torcs_output[ACCEL_VALUE] = 1.0;
	else
		torcs_output[ACCEL_VALUE] = re;
	if(torcs_output[CC_TRIGGER] > 0.5)
		printf("target speed: %f, cur speed: %f, re: %f\n", torcs_output[TARGET_SPEED], torcs_output[SPEED], re);
	torcs_output[SPEED] = 3.6*car->_speed_x;
	torcs_output[STEER_VALUE] = angle/car->_steerLock;
	torcs_output[RPM] = car->_enginerpm*10.0;
	torcs_output[YAW_RATE] = car->_yaw_rate;
	...
	switch (cmd[CMD_LEFTSTEER].type) {
		...
		case GFCTRL_TYPE_JOY_BUT:
			if (cmd[CMD_LEFTSTEER].type == GFCTRL_TYPE_KEYBOARD) {
				...
				/*** kswe ***/
				if(user_input[LEFT_STEER] > 0.01)
				{
					ax0 = user_input[LEFT_STEER];
				}
			//	printf("ax0: %f, simul_time: %f, real_time: %f\n", ax0, s->currentTime, ReInfo->_reCurTime);
			//	printf("pos: %f, steer_angle: %f, steering mode: %f\n", car->_trkPos.toMiddle, torcs_output[3], user_input[4]);
				printf("time: %f, speed: %f, rpm: %f, yaw_rate: %f, dist: %f, steering: %f\n", torcs_output[PASSED_TIME], 3.6*car->_speed_x, car->_enginerpm*10.0, car->_yaw_rate, torcs_output[DISTANCE], torcs_output[STEER_VALUE]);
				/************/
			} else {
	...
	switch (cmd[CMD_RIGHTSTEER].type) {
		...
		case GFCTRL_TYPE_JOY_BUT:
			if (cmd[CMD_RIGHTSTEER].type == GFCTRL_TYPE_KEYBOARD) {
				...
				/*** kswe ***/
				if(user_input[RIGHT_STEER] > 0.01)
				{
					ax0 = user_input[RIGHT_STEER];
				}
				/************/
			} else {
	...
	/*** kswe ***/
	if(user_input[RIGHT_STEER] > 0.01)
		rightSteer = -user_input[RIGHT_STEER];
	if(user_input[LEFT_STEER] > 0.01)
		leftSteer = user_input[LEFT_STEER];
	/************/
